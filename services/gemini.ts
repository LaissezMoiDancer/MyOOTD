...
// Weather: always uses the backend (Open-Meteo, free, no Gemini quota used)
// If the backend is unreachable, throws an error instead of falling back to Gemini
export const fetchWeather = async (location: string): Promise<{
  context: string,
  type: 'sunny' | 'windy' | 'snowy' | 'cloudy' | 'raining',
...
// Gemini call #1: analyze the clothing photo
    ...
    contents: {
      parts: [
        ...
        },
        {
          ...
          - bodyType: (Which body types this specific piece flatters most)
...
// Gemini call #2: generate outfit recommendations
