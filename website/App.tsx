...
const SeasonalParticles: React.FC<{ season: 'spring' | 'summer' | 'autumn' | 'winter' | null, currentStep: AppStep }> = ({ season, currentStep }) => {
  // Allow animations on steps once season is detected. Block only on wardrobe view.
  if (!season || currentStep === 'wardrobe') return null;

  const particles = Array.from({ length: 30 }); // Increased particle count for better visibility
...
const WeatherAnimation: React.FC<{ type: string }> = ({ type }) => {
  ...
  switch (type) {
    ...
    case 'windy':
      return (
        <div className={baseClass}>
          <Wind className="w-20 h-20 text-stone-300 animate-pulse" />
        </div>
      );
    case 'cloudy':
    default:
    ...
    <div className="max-w-7xl mx-auto py-10 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 relative z-10">
      ...
      <div className="grid lg:grid-cols-2 gap-12 items-start -mt-4">
        <div className="flex flex-col pt-24">
          ...
          <div className="flex items-center space-x-16 px-10 pt-12">
            <div className="flex flex-col items-center space-y-4 opacity-20 hover:opacity-100 transition-opacity">
               <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M7 3L5 21H19L17 3H7Z"/><path d="M12 3V21"/><path d="M7 8H17"/></svg>
               <span className="text-[11px] font-black uppercase tracking-[0.4em] text-stone-500">Jeans</span>
            </div>
            <div className="flex flex-col items-center space-y-4 opacity-20 hover:opacity-100 transition-opacity">
               <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3L3 9V21H21V9L18 3H6Z"/><path d="M3 9H21"/><path d="M9 3V9"/><path d="M15 3V9"/></svg>
               <span className="text-[11px] font-black uppercase tracking-[0.4em] text-stone-500">Dress</span>
            </div>
            <div className="flex flex-col items-center space-y-4 opacity-20 hover:opacity-100 transition-opacity">
               <Shirt className="w-16 h-16" strokeWidth={1.2} />
               <span className="text-[11px] font-black uppercase tracking-[0.4em] text-stone-500">Shirt</span>
            </div>
            <div className="flex flex-col items-center space-y-4 opacity-20 hover:opacity-100 transition-opacity">
               <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 3H20V21H4V3Z"/><path d="M4 8H20"/><path d="M10 8V21"/><path d="M14 8V21"/><path d="M20 3L22 8V21"/><path d="M4 3L2 8V21"/></svg>
               <span className="text-[11px] font-black uppercase tracking-[0.4em] text-stone-500">Jacket</span>
            </div>
          </div>
        </div>

        <div className="flex flex-col items-center justify-center p-12 bg-white rounded-[4rem] border border-stone-100 min-h-[400px] shadow-2xl relative overflow-hidden group w-full">
    ...
    <div className="max-w-4xl mx-auto py-10 space-y-16 animate-in fade-in slide-in-from-right-10 duration-1000 relative z-10">
      ...
      <div className="max-w-xl mx-auto space-y-10">
         <div className="bg-white p-12 rounded-[3.5rem] border border-stone-100 space-y-8 shadow-sm">
            <label className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-400 text-center block uppercase">Accent Palette Target</label>
