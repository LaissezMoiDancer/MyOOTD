...
const StepCloset: React.FC<{ 
  wardrobe: UserPhoto[],
  // FIX: added selectedItem prop so a pre-selected item from the wardrobe view is shown
  selectedItem: UserPhoto | null,
  onBack: () => void, 
  onNext: (photo: UserPhoto | null, color: string) => void 
}> = ({ wardrobe, selectedItem, onBack, onNext }) => {
  const [color, setColor] = useState(STANDARD_COLORS[0].hex);
 
  const handleSubmit = () => {
    onNext(selectedItem, color);
  };
 
  const canSynthesize = wardrobe.length > 0;
 
  // Show which item will be analyzed
  const itemToAnalyze = selectedItem || wardrobe[0];
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 space-y-16 animate-in fade-in slide-in-from-right-10 duration-1000 relative z-10">
      ...
      <div className="max-w-xl mx-auto space-y-10">
        {/* FIX: show which item will be analyzed so the user knows what's selected */}
        {itemToAnalyze && (
          <div className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm flex items-center space-x-6">
            <img src={itemToAnalyze.url} className="w-20 h-20 object-contain rounded-xl border border-stone-100" alt="Selected item" />
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-stone-400 mb-1">
                {selectedItem ? 'Selected Piece' : 'First Wardrobe Item'}
              </p>
              <p className="text-sm text-stone-600 font-light">This piece will be analyzed by the AI stylist.</p>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  // FIX: track a specifically selected item separately from the auto-picked wardrobe[0]
  ...
  // FIX: uploadedPhoto is now the explicitly selected item (passed from StepCloset)
  // falling back to wardrobe[0] only if nothing was selected
  ...
  const deleteClosetItem = (id: string) => {
    ...
    // FIX: clear selectedItem if the deleted item was selected
    if (selectedItem?.id === id) setSelectedItem(null);
  };
 
  // FIX: store the selected item in state and navigate to the flow start
